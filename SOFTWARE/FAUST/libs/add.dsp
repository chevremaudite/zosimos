...
add(trig) = hgroup("[1]Addictive", additive(pitch, ratioList, gainList)*(trig:volEnv))
with{
    ...
    with {
        oddness = vslider("[4]Oddness[style:knob]", 1, 0, 200, 0.001);
        interval = vslider("[3]Harm interval[style:knob]", 0, 0, 0.99, 0.001);
    };
    ...
    harms = vslider("[2]Harm number[style:knob]", 0.2, 0, 0.6, 0.001);
